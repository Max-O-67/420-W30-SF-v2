# E04 — Automatiser l'assemblage avec le conteneur .NET

## Mission et durée

En 60 minutes, partez de la solution réussie en E03 et remplacez l'assemblage
manuel de la racine de composition par le moteur d'injection de dépendances.

## 1. Créer la branche fonctionnelle

Revoyez [les commandes de E13](./E13-branches-git.md) au besoin.
...
Les tests de E03 doivent continuer à construire directement le service avec
leur doublure mémoire.

```mermaid
flowchart LR
    R[Enregistrements dans Program.cs] --> C[Conteneur .NET]
    C --> N[NotificationConsole]
    C --> S[ServiceCommandes]
    N --> S
    S --> A[Création de la commande]
```
