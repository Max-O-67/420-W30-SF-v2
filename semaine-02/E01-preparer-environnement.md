...
> noire progressivement dans E03 et E04.
...
1. Créez une **solution vide** nommée
   `S02E01_Restaurant_Commande`.
2. Ajoutez une **Bibliothèque de classes** nommée
   `S02E01_Restaurant_Commande`, ciblant **.NET 10.0**.
3. Ajoutez une **Application console** nommée
   `S02E01_Restaurant_Commande.Terminal`, ciblant **.NET 10.0**.
4. Ajoutez un **Projet de test xUnit** nommé
   `S02E01_Restaurant_Commande.Tests`, ciblant **.NET 10.0**.
...
Dans `S02E01_Restaurant_Commande.Terminal`, puis dans
`S02E01_Restaurant_Commande.Tests` :

1. cliquez avec le bouton droit sur **Dépendances**;
2. choisissez **Ajouter une référence de projet**;
3. cochez `S02E01_Restaurant_Commande`.

Dans `S02E01_Restaurant_Commande.Tests` seulement :
...
Dans `S02E01_Restaurant_Commande`, créez `IExpediteurMessage.cs` :
...
Dans `S02E01_Restaurant_Commande.Terminal`, remplacez `Program.cs` par ce test
...
`S02E01_Restaurant_Commande.Tests`, utilisez ensuite ce test :
...
[E02 — Tests sans dépendance](./E02-tests-sans-dependance.md).
